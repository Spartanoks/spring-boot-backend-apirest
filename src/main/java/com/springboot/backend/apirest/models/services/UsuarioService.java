...
public class UsuarioService implements UserDetailsService, IUsuarioService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (usuario == null) {
            logger.error("Error en el login: el usuario " + username + "no extiste en el sistema ");
            throw new UsernameNotFoundException(
                    "Error en el login: el usuario " + username + "no extiste en el sistema ");
        }

        List<GrantedAuthority> authorities = usuario.getRoles().stream()
                .map(role -> new SimpleGrantedAuthority(role.getNombre()))
                .peek(authority -> logger.info("Role: " + username)).collect(Collectors.toList());
